...
async def startup_event():
    ...
    try:
        ...
        for demo_device in demo_devices:
            ...
            # Ensure both admin and demo user can view demo devices.
            for demo_account in [admin_user, user_user]:
                existing_permission = db.query(UserDevicePermission).filter(
                    UserDevicePermission.user_id == demo_account.id,
                ...
                if not existing_permission:
                    ...
                    db.add(permission)
                    db.commit()
                    print(f"✓ [Startup] Granted {demo_account.username} access to {device.name}")
